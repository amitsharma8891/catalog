  ...
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          <div class="card-body">
            <form class="col s12" method="POST" action="" @submit.prevent="createProduct" autocomplete="off">
              <div class = "row">
                <div class="form-group m-t-20 col-md-6">
                  <label>Title</label>
                    <input id="first_name" v-model="fields.title" placeholder="Product Title" type="text" class="form-control phone-inputmask validate" required>
                  </div>
                  <div class="form-group m-t-20 col-md-6">
...
export default {
  data () {
    return {
      categories: [],
      fields: { title: '', category_id: '', original_price: 0.00, discount_price: 0.00, in_stock: 1, status: 1 },
      errors: {}
    }
  },
  computed: {
  ...
  methods: {
    createProduct () {
      this.$store.dispatch('CREATE_PRODUCT', this.fields).then(response => {
        alert(response.data)
